...
The root workspace is responsible for smart contract compilation, testing, and deployment. The `web/` workspace is a separate Next.js application that talks to the deployed contracts.

## Smart Contracts

### `ChainInvite`

`contracts/ChainInvite.sol` is the standard invite contract.
...
`contracts/ChainInviteNFT.sol` extends the same event model with ERC-721 ticketing.
