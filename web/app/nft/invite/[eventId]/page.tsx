...
import { AlertTriangle, ArrowLeft, BadgeCheck, ExternalLink, Loader2, RefreshCw } from "lucide-react";
...
export default function InvitePage() {
  ...
  const state: InviteState = useMemo(() => {
    ...
    if (tokenId && tokenId > 0n && validInvite === false) {
      return "used";
    }

    return "missing";
  }, [isConnected, isSepolia, tokenId, validInvite]);

  const qrPayload = useMemo(() => {
    if (!address || !tokenId || tokenId === 0n) {
      return null;
    }
  ...
  }, [address, params.eventId, tokenId]);

  useEffect(() => {
    let cancelled = false;

    async function makeQrCode() {
      if (state !== "valid" || !qrPayload) {
        setQrDataUrl(null);
        return;
      }
  ...
  const isLoading = isEventLoading || isTokenLoading || isInviteLoading;

  return (
    <main className="min-h-screen px-4 py-6 text-[#1d2527] md:px-8">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-5">
        <PageHeader eyebrow="NFT Invite" title={`Event #${params.eventId}`} />
          ...
          <div className="rounded-md border border-[#d8d2c6] bg-white p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <h2 className="mt-3 text-2xl font-semibold">
                  {state === "valid" ? "Valid NFT invite" : null}
            ...
            {state === "valid" && qrDataUrl ? (
              <div className="mt-5 grid gap-4">
                <div className="rounded-md bg-[#e9f5ee] p-3 text-sm font-semibold text-[#1d7b4f]">
                  Ticket NFT #{tokenId?.toString()}
                </div>
                ...
                <div className="rounded-md bg-[#f6f4ee] p-3 font-mono text-xs leading-5 text-[#34413f]">
                  {qrPayload ? JSON.stringify(qrPayload) : null}
                </div>
                {tokenId ? (
                  <a
