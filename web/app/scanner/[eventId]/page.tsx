...
function parseScannerPayload(raw: string, expectedEventId: bigint): ScannerPayload {
  ...
  const payload = parsed as { eventId?: unknown; guest?: unknown };
...
export default function ScannerPage() {
  ...
  const {
    ...
    args: scannedPayload ? [eventId, scannedPayload.guest] : undefined,
    chainId: sepolia.id,
    query: {
  ...
  function checkInGuest() {
    ...
    writeContract({
      ...
      args: [eventId, scannedPayload.guest],
      chainId: sepolia.id,
    });
  }
  ...
  return (
    <main className="min-h-screen px-4 py-6 text-[#1d2527] md:px-8">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-5">
        <PageHeader eyebrow="Scanner" title={`Event #${params.eventId}`} />
